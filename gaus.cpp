//void plott()
{
     ...
     double x;
     TF1 *f1 = new TF1("f1", "[0]*TMath::Gaus(x,[1],[2])", 0, 4); //フィッティングする為の関数の定義。[]で囲んである部分がフィットで変化させるパラメーター。関数を使うときはTMath::expのようにする。TMath::Gaus(Double_t x,Double_t mean,Double_t sigma)
     ...
     h1->Fit("f1", "", "", 0, 4);                          //xが0.000000001から10の範囲についてフィット
