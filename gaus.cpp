//void plott()
{
     ...
     TH1F *h1 = new TH1F("h1", "", 2000, 0, 25);
     v1290->Draw("0.02441*(drift[9]-drift[6])-12 >> h1"); //h(指定範囲の分割数,左端,右端)
     double x;
     TF1 *f1 = new TF1("f1", "[0]*TMath::Gaus(x,[1],[2])", 0, 3); //フィッティングする為の関数の定義。[]で囲んである部分がフィットで変化させるパラメーター。関数を使うときはTMath::expのようにする。TMath::Gaus(Double_t x,Double_t mean,Double_t sigma)
     f1->SetNpx(10000);                                           //フィットした関数をプロットする細かさ

     f1->SetParameters(1, 0, 1); //パラメータの初期値を決める

     f1->SetParNames("constant", "mean[ns]", "sigma[ns]"); //パラメータの名前を決める
     h1->Fit("f1", "", "", 0, 3);                          //xが0.000000001から10の範囲についてフィット

     f1->Draw("same");    //読み込んだデータに重ねて書く
     c1->Print("03.png"); //xxx.pngの形式で書き出す
}
